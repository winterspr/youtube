...
const Navbar = ({ setSideBar }) => {
  ...
  // Sử dụng useRef để lưu tham chiếu đến dropdown menu và search box
  const dropdownRef = useRef(null);
  const searchBoxRef = useRef(null);

  // Hàm xử lý khi click vào profile, hiển thị hoặc ẩn dropdown
  const handleProfileClick = () => {
    setShowDropdown((prev) => !prev);
  };

  // Hàm xử lý khi click vào nút Sign Out
  const handleSignOut = () => {
    dispatch(logout());
    setShowDropdown(false);
    navigate("/signin");
  ...
  return (
    <nav className="flex-div">
      ...
      <div className="nav-middle flex-div">
        <div className="search-box flex-div" ref={searchBoxRef}>
          <input
            type="text"
            placeholder="Search"
            value={searchQuery}
          ...
          <div className="search-results-dropdown">
            {searchResults.map((result) => (
              <div
                key={result._id}
                className="search-result-item"
                onClick={() => handleVideoClick(result._id)}
              >
                <img src={result.thumbnails.medium.url} alt={result.title} />
                <div>{result.title}</div>
              </div>
